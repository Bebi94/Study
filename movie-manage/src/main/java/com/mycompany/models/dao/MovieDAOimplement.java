...
public class MovieDAOimplement implements MovieDAO {
    ...
    public void MovieDAOImpl(Connection connection) {
    ...
    public void createMovie(Movie movie) throws DatabaseActionException {
        String sql = "INSERT INTO MOVIES (title, genre, yearOfRelease) VALUE (?,?,?)";
    ...
    @Override
    public void updateMovieTitle(int id, String newTitle) throws DatabaseActionException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    rt.add(mapResultSetToMovie(resultSet));
                }
            }
            return rt;
        } catch (SQLException e) {
